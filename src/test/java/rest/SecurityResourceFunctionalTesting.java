...
public class SecurityResourceFunctionalTesting {
    ...
    private static final String url = "http://localhost:8080/springProject.0.0.1-SNAPSHOT/api/v0";
    ...
    public void testUserAnnotationOK() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path("/securityAnnotation").basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
    
    @Test
    public void testUserURIOK() {
        String response = new RestBuilder<String>(url).path(Uris.SECURITY).path("/securityUri").basicAuth("u1", "123456")
                .clazz(String.class).get().build();
        System.out.println("INFO >>>>> " + response);
    }
    
    @Test
    public void testManagerUnauthorizedUserAnnotation() {
        try {
            String response = new RestBuilder<String>(url).path(Uris.SECURITY).path("/securityAnnotation").basicAuth("u1", "12345")
                    .clazz(String.class).get().build();
            System.out.println("INFO >>>>> " + response);
            fail();
    ...
    @Test
    public void testManagerUnauthorizedUserURI() {
        try {
            String response = new RestBuilder<String>(url).path(Uris.SECURITY).path("/securityUri").basicAuth("u1", "12345")
                    .clazz(String.class).get().build();
            System.out.println("INFO >>>>> " + response);
            fail();
        } catch (HttpClientErrorException httpError) {
